...
def _read_sql(sql: str, params=None) -> pd.DataFrame:
    ...
    with _engine.connect() as con:
        if params:
            return pd.read_sql_query(sa_text(adapted) if not _IS_POSTGRES else adapted,
                                     con, params=list(params) if _IS_POSTGRES else params)
        return pd.read_sql_query(adapted, con)
